...
- **Criticidad:** alta
- **Bloquea a:** ~~Fase 2 (deploy a producción con respaldo legal)~~ — ya no bloquea. **El único gate que queda para el switch a PÚBLICO es P-51.**
...
- **Estado:** 🔴 BLOQUEADO por tarea manual de Francisco (alta del 04/08/2026, rama `publicacion-v1`). **Desde el 09/08/2026 es el ÚNICO gate abierto del switch a PÚBLICO**: P-39 cerró como R-18 y P-42 como R-17, así que no queda nada más entre el sitio y la publicación. **Criterio de cierre: CONFIGURAR SIN PROBAR NO CUENTA.** El ítem se cierra cuando un correo enviado desde fuera llegue efectivamente a la bandeja de Francisco, no cuando la regla aparezca creada en Cloudflare.
...
- **Consecuencia para el lanzamiento:** con P-39 y P-42 cerrados, **P-51** (casilla `privacidad@barreraglobal.com` configurada **y probada**) queda como **único gate abierto** del switch a público.
